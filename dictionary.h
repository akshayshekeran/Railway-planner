...
public:
  int adjacentStnIndex;
  listOfObjects<TrainInfoPerStation *> *trains;

  StationConnectionInfo(int index = -1) {
    adjacentStnIndex = index;
    trains = nullptr;
  }

  ~StationConnectionInfo() {
    listOfObjects<TrainInfoPerStation *> *currTrain = trains;

    // Free up all elements in the list of train info per station
    // (as stored in the "trains" list),
    // without actually deleting the real TrainInfoPerStation data.
...
class StationAdjacencyList {
public:
  listOfObjects<StationConnectionInfo *> *toStations;
  listOfObjects<StationConnectionInfo *> *fromStations;

  StationAdjacencyList() {
    toStations = fromStations = nullptr;
  }

  ~StationAdjacencyList() {
    listOfObjects<StationConnectionInfo *> *currStnConnInfo;
    listOfObjects<StationConnectionInfo *> *delStnConnInfo;

    currStnConnInfo = toStations;
